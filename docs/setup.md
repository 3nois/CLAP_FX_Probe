...
[5차](02_resolution_and_family.md) 이후 라운드(`09_fd_phase0.py`\~`23_report_figures.py`)의 스크립트는 각 라운드 문서
(`docs/02_resolution_and_family.md`, `docs/03_direction_prediction.md`, `docs/04_tokensynth_generation.md`, `docs/05_projection_diagnosis.md`,
...
`scipy`, `soundfile`, `matplotlib`가 추가로 설치됩니다. [4차](01_probe_baseline.md) 개정은 새 의존성이 없습니다 —
...
되도록 보장한다 (`neutral_check` 참고 — [4차](01_probe_baseline.md) 재실행에서도 세 이펙트 전부 정확히 1.000).
...
단계입니다(45,600개 CLAP forward pass, [4차](01_probe_baseline.md)에서는 스킵). `02`는 마스킹 변형별로 완전히
새로 학습하므로(5회) [3차](01_probe_baseline.md)의 단일 학습보다 느립니다(M5 CPU 기준 40\~50분). `03`\~`08`은 모두
...
`out/`는 [6차](02_resolution_and_family.md) 후속에서 산출물이 40여 개로 늘어나며 **유형별 하위 폴더로 정리**했다
...
★ **caches 정책이 한 번 바뀌었다.** `oat_emb.npz`는 원래 "[2차](01_probe_baseline.md) 데이터 소실 재발
방지용으로 의도적으로 커밋 대상"으로 의도했으나([7차](02_resolution_and_family.md) 작성 당시), 실제로는 한 번도
...
`subspace`, `quality_stratification`. `results_2.json`은 [2차](01_probe_baseline.md) 당시의 원본 산출물이며
이후 라운드의 `results.json` 재실행으로 덮어써져 로컬에서 소실됐다가 git 이력
(커밋 `817ccca`)에서 복원했다 — `effects.*.probe_nmi`(이펙트별 NMI)가 이 파일에만
있다. `results_5/6/7/8/9_*/10_projection.json`은 각 라운드 문서(`docs/0*.md`)에서
