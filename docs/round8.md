...
[7차](round6-7.md)(`out/results_7.json`)에서 손잡이 방향이 소스마다 다르다(within > between)를
...
양쪽에서 절대 섞지 않는다. 재렌더링 0회 — `out/caches/oat_emb.npz`([7차](round6-7.md), 1,200소스×
3레벨×3이펙트, 조건A)만 읽는다.

## 사전 등록 (실행 전 확정, 결과를 본 뒤 바꾸지 않음)

**기준선** ([7차](round6-7.md) 근거): 전역 평균 방향 cos ≈ 0.24, 악기군 평균 방향(오라클) cos ≈ 0.34
...
★ **사전 예상과 다른 결과** — "[5차](round6-7.md) 역방향 모델이 이 지점에서 실패했으므로(param R²
...
상대오차 손실로 직접 학습)가 [5차](round6-7.md) 역방향 모델(다른 아키텍처·다른 손실)보다 훨씬 나은
...
바꾸는 이펙트라는 [6차](round6-7.md) 이전 결과(distortion.drive_db 신호 최강)와 일관된다.
