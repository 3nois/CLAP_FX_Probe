...
- `freeze_mode=1`이 reverb 4개 연속 파라미터를 완전 무효화한다 ([3차](round1-4.md) 25,600행 중 48.7%가
  무효 표본). freeze=0 층화 시 reverb 프로브 R²가 3.4\~4.8배 상승했다.
- `width`는 음성 통제가 아니다 (freeze=0에서 R²=0.066, CI가 0을 명확히 벗어남) — [4차](round1-4.md)
...
| `surrogate` | 대리모델 J. 불신 (pooled cos(J_fd, J_surrogate)=0.418) | 3·[4차](round1-4.md)의 해당 항목 |
...
**새 해상도 바닥 = max(−0.0047, −0.0017) = −0.00172** (음수). [4차](round1-4.md) 바닥(0.0096, width
...
값이고, 재분류 대상 대부분은 N=6,400\~25,600(3·[4차](round1-4.md)/과제 1, 조건 A)에서 나온 값이었다.
...
highshelf(0.045). 조건 A·N=500 매칭 서브샘플(3·[4차](round1-4.md) 원본 데이터에서 src_id 단위 100회
...
**"측정 불가"**. [4차](round1-4.md) 원본(조건 A, range 500\~8000, N=12,800)의 R²=0.0039/0.0142와는
N·range가 모두 달라 직접 비교하지 않는다.

**[4차](round1-4.md) "측정 불가" 2건 재검토**: `reverb.width`, `highshelf.cutoff_frequency_hz` 모두
...
- **highshelf.cutoff/q (조건 C, range 500\~4000)**: 단독·C·500에서도 널과 겹침. 이건
  N 문제만이 아닐 수 있다 — [4차](round1-4.md) 원본(N=12,800, range 500\~8000)에서도 R²가 0.004/0.014로
...
N=500에만 있었다. 초음파 축만 [3차](round1-4.md) highshelf와 같은 규모·조건으로 다시 렌더링해
(800소스×16θ=12,800점, **조건 A**·피크 0.7, **단독 적용**, `out/ultrasonic_null_largeN.npz`)
큰 N 널을 만들고, [3차](round1-4.md)/과제1과 완전히 같은 프로브 절차(Ridge α=1.0, GroupShuffleSplit,
...
★ **highshelf.q도 이번엔 신호로 확정**([4차](round1-4.md) 스칼라 바닥 0.0096 기준으로는 애매했으나
...
**과제 6 — family cosine** (판정 기준은 [3차](round1-4.md)와 동일하게 유지)
...
[2차](round1-4.md)(OAT) 임베딩이 디스크에서 소실된 것을 확인했다(`out/embeddings.npz`가 gitignore
대상이면서 [3차](round1-4.md) 이후 같은 파일명으로 덮어써짐 — git에도 한 번도 커밋된 적 없음). 그대로
...
highshelf 반쪽 스윙 cos(0→+9, −9→0) = **0.764**(둘 다 "증가" 방향으로 정의 — [2차](round1-4.md)의
`cos(v+,v−)=−0.955`와는 부호 정의가 다르다: [2차](round1-4.md)는 v−를 "감소" 방향(e(−9)−e(0))으로
...
| 이펙트 | 원값(45쌍 평균) | 보정값 | self_cosine<0.7 패밀리 | [3차](round1-4.md)(대리모델,불신) | 대표 파라미터 |
|---|---|---|---|---|---|
...
공유(공통 성분 + 악기 고유 성분)"**로 일관되게 분류된다. [3차](round1-4.md) 값(대리모델 기반, 불신
...
surrogate-free 재현과 부합한다. **[3차](round1-4.md)의 family cosine 결론(공통 손잡이 하나로는
...
## 과제 7 — 보류 결론 복구
`v = mean(e_wet − e_dry)`([3차](round1-4.md) 전체 표본, reverb는 freeze=0만)로 교체. 7-B/7-C는 J_fd
기반으로 재산출하고 [4차](round1-4.md) 값과 비교표로 제시(단정하지 않음).
