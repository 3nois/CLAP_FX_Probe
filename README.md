...
CLAP 임베딩은 악기 정체성은 강하게 담지만 이펙트 정보는 3.4~9.2배 약하게만 담는다(1~2차).
그 약한 신호로도 손잡이 방향은 코사인 0.71~0.86으로 상당히 정확하게 예측된다(7~8차). 그러나
그 방향으로 임베딩을 이동시켜 실제 TokenSynth 오디오를 생성하면 방향 일치도가 코사인
0.03~0.06(무작위에 가까움)까지 무너진다(9차) — projection layer를 진단해보면 이펙트 정보가
이펙트마다 다르게 선택적으로 약화되고 악기 정보는 오히려 강화되는, 단순 "압축"으로는 설명 안
되는 패턴이 나온다(10차). **정보가 없는 것이 아니라, 예측까지는 되는데 TokenSynth를 통과하면서
거의 다 소거된다.**
...
모든 수치는 코드가 산출하고, 판정은 각 문서 안에 사람이 기록한다(코드가 결론을 내리지 않는다).
